...
def list_ollama_models() -> list[str]:
    try:
        ...
        out = subprocess.check_output(["/usr/local/bin/ollama", "list"], text=True, stderr=subprocess.STDOUT)
    except Exception as e:
                ...
                with left:
                    ...
                    if stored_path.suffix.lower() == ".pdf":
                        ...
                        if imgs:
                            ...
                            if num_pages > 1:
                                page0 = st.slider("Page", min_value=1, max_value=num_pages, value=1, key=f"page_{fid_choice}")
                            else:
                                page0 = 1
                                st.write("Page: 1")
